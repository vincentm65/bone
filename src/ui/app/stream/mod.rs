...
impl App {
    ...
    pub(super) async fn submit_user_turn(
        ...
        // Opt-in: run the turn through the core `Driver` instead of this
        // module's bespoke loop. Gated by `BONE_DRIVER` so the proven path stays
        // default until the new one is verified (Step 3 cutover).
        if std::env::var_os("BONE_DRIVER").is_some() {
            return self
                .submit_user_turn_via_driver(text, display_text, term)
                .await;
        }
        self.messages
        ...
        loop {
            ...
            {
                let ctx_cfg = crate::ext::ctx::build_before_turn_config(&self.app_ctx_state());
            ...
            // (Re)build history from the (possibly compacted) transcript.
    ...
    /// Gated cutover path (Step 3): run the turn through the core `Driver` and
    /// render its `RuntimeEvent` stream, answering approval/interaction over
    /// channels. The Driver owns clones of `tools`/`extensions` (Lua VM shared
    /// via Arc) and `Arc` provider/session; it runs as a local future pumped by
    /// a `select!` loop on this task — so there's no spawn, no borrow conflict,
    /// and (crucially) the render path never touches the Lua VM while a tool
    /// runs lock-free. State (transcript/token_stats/tool-state) is reabsorbed
    /// from the returned `DriverOutcome`.
    ///
    /// v1 limitation (gated): no per-turn DB persistence.
    pub(super) async fn submit_user_turn_via_driver(
        &mut self,
        text: String,
        display_text: Option<String>,
        term: &mut BoneTerminal,
        ...
        > = std::collections::HashMap::new();
        let mut ticker = tokio::time::interval(std::time::Duration::from_millis(30));
        ...
        if let Some(outcome) = outcome {
            ...
            // (so `BONE_DRIVER` turns appear in /history). Clone the slice out
    ...
    fn pump_apply_event(
        ...
        match ev {
            RuntimeEvent::TextDelta { text } => {
                ...
                let est = self.token_stats.received;
                self.redraw_streaming_tokens(idx, est, term)?;
            }
