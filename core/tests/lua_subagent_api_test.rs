...
"#,
    ...
    let _config =
        bone_core::config::store::ConfigStore::for_test_with_extensions(booted.manager.clone());
...
assert(agents[2].name == "lua-only" and agents[2].source == "lua" and agents[2].max_concurrency == nil)
assert(agents[3].name == "shared" and agents[3].source == "config" and agents[3].max_concurrency == nil)
