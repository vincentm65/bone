            ...
            match returned {
                Ok(mlua::Value::Table(table)) => {
                    ...
                    if let Some(action) = parse_lua_return_action(&table, name == "before_turn") {
                        if name == "before_turn"
                            && let Some(append) = action.system_prompt_append.as_deref()
                        {
                            let base =
                            ...
                            ctx_cfg.system_prompt_override = Some(format!("{base}\n\n{append}"));
                            let lua = lua_handle.lock().unwrap_or_else(|error| error.into_inner());
    ...
    match value {
        ...
        mlua::Value::Table(t) => {
            let output = t
                ...
                .unwrap_or_default();
            let action = parse_lua_return_action(&t, false);
...
pub(crate) fn parse_lua_return_action(
    ...
    // compatibility. The command projection intentionally drops them.
    if let Some(s) = table
